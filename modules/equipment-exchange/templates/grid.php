...
<section class="oras-equipment-grid-page">
	...
	<form method="get" class="oras-equipment-grid-filters">
		...
		<p><label><?php esc_html_e( 'Listing type', 'oras-member-hub' ); ?> <select name="listing_type"><option value=""><?php esc_html_e( 'All', 'oras-member-hub' ); ?></option><?php foreach ( ORAS_MH_Equipment_Fields::listing_types() as $filter_type_key => $filter_type_label ) : ?><option value="<?php echo esc_attr( $filter_type_key ); ?>" <?php selected( (string) ( $filters['listing_type'] ?? '' ), $filter_type_key ); ?>><?php echo esc_html( $filter_type_label ); ?></option><?php endforeach; ?></select></label></p>
		<p><label><?php esc_html_e( 'Category', 'oras-member-hub' ); ?> <select name="category"><option value="0"><?php esc_html_e( 'All', 'oras-member-hub' ); ?></option><?php foreach ( $categories as $filter_category ) : ?><option value="<?php echo esc_attr( (string) $filter_category->term_id ); ?>" <?php selected( (int) ( $filters['category'] ?? 0 ), (int) $filter_category->term_id ); ?>><?php echo esc_html( $filter_category->name ); ?></option><?php endforeach; ?></select></label></p>
		<p><label><?php esc_html_e( 'Condition', 'oras-member-hub' ); ?> <select name="condition"><option value="0"><?php esc_html_e( 'All', 'oras-member-hub' ); ?></option><?php foreach ( $conditions as $filter_condition ) : ?><option value="<?php echo esc_attr( (string) $filter_condition->term_id ); ?>" <?php selected( (int) ( $filters['condition'] ?? 0 ), (int) $filter_condition->term_id ); ?>><?php echo esc_html( $filter_condition->name ); ?></option><?php endforeach; ?></select></label></p>
		<p><label><?php esc_html_e( 'Status', 'oras-member-hub' ); ?> <select name="status"><option value=""><?php esc_html_e( 'All', 'oras-member-hub' ); ?></option><?php foreach ( ORAS_MH_Equipment_Fields::all_public_status_labels() as $status_key => $status_label ) : ?><option value="<?php echo esc_attr( $status_key ); ?>" <?php selected( (string) ( $filters['status'] ?? '' ), $status_key ); ?>><?php echo esc_html( $status_label ); ?></option><?php endforeach; ?></select></label></p>
